/*
        ...
        CREATED BY: RAMONA SARTIPI - 2018
...
hull(){
    translate([r_front, r_front, r_front-1.7])
    {
        sphere(r = r_front);
    }
    
    translate([x_out - r_front, r_front, r_front-1.7])
    {
        sphere(r = r_front);
    }
}
...
module slider(x_slid, y_slid, z_slid, r_slid)
{
    ...
    translate([(x_out/2)-(x_slid/2), (0.9)*r_front, 1.7*r_front]) 
    {
        ...
        hull(){
            translate([r_front, r_front, 0])
            {
                cylinder(r = r_front, h = z_slid);
            }
            
            translate([x_slid - r_front, r_front, 0])
            {
                cylinder(r = r_front, h = z_slid);
            }
            
            translate([r_front, y_slid - r_front, 0])
            {
                cylinder(r = r_front, h = z_slid);
            }
            
            translate([x_slid - r_front, y_slid - r_front, 0])
            {
                cylinder(r = r_front, h = z_slid);
            }
        }
...
{
    //2nd item changes gap between braille and bump
    translate([(x_out/2)-(x_cell/2), (1.48*r_front), (1.6*r_front)])
    {
...
{
    translate([(x_out/2)-(x_slid/2),(1.3*r_front+gap)+(y_cell/2), (1.75*r_front)])
    {
        lid_front(x_guide, y_guide, r_guide);
    }
    
    translate([x_out-(x_slid/10)- x_guide,(1.3*r_front+gap)+(y_cell/2), (1.75*r_front)])
    {
        lid_front(x_guide, y_guide, r_guide);
    }
}
...
{
    translate([r_front, (0.57)*r_front, -(0.5)*z_bplate])
    {
...
{
    //button1
    #translate([r_front+ (x_bplate/18)+((x_bplate/3)-x_but)/2, 0, -z_but/4])
    {
        rotate ([270,0,0]) cylinder(r = 8.2, h =r_front);     
    }
    //button2
    translate([r_front+ (x_bplate/3.6)+((x_bplate/3)-x_but)/2, 0, -z_but/4])
    {
        rotate ([270,0,0]) cylinder(r = 8.2, h =r_front); 
    }
    //button3
    translate([r_front+ (x_bplate/2)+((x_bplate/3)-x_but)/2, 0, -z_but/4])
    {
        rotate ([270,0,0]) cylinder(r = 8.2, h =r_front); 
    }
   
    //button4
    #translate([r_front+ (x_bplate/1.4)+((x_bplate/3)-x_but)/2, 0, -z_but/4])
    {
        rotate ([270,0,0]) cylinder(r = 8.2, h =r_front); 
    }
}
...
module lid_v3_att3()
{
    difference()
    {
        //bottom cut-out of the front (for the braille cell to fit)
        ...
        //side cut-out of the front (for the braille cell to fit)
        #translate([20, r_cut+5, r_front/4])cube([x_out-40, r_front, r_front+10]);
    }
    difference()
    {
    ...
    guide_line(x_guide, y_guide, r_guide);
     ...
     #translate([r_slid + 5, y_out - r_slid - 8,  1.7*r_slid+z_slid])iso_std_braille_alpha("ABCDEFGHIJKLM", 8);
    
    #translate([r_slid + 5, y_out - (1.7)*r_slid - 8,  1.7*r_slid+z_slid])#iso_std_braille_alpha("NOPQRSTUVWXYZ", 8);
